...
- **Dual-radio BLE** (Meshtastic + MeshCore different peripherals): concurrent sidecar GATT sessions; wake/startup stagger via `lib/meshcoreDualNobleBleInit.ts` (historical name) from `App.tsx` `useLayoutEffect` — see [docs/agents/ble-serial.md](docs/agents/ble-serial.md).
