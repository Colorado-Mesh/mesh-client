...
- Mid-session macOS `EXC_BREAKPOINT` / SIGTRAP after multi-day Noble BLE is a **native Noble / Electron main-process teardown race** (timer tick intersecting V8 GC). It is **outside mesh-client’s JavaScript control** — not a corrupt database and not catchable with `try/catch`. Mitigation is process recycle (restart) and preferring Serial/TCP for always-on desks.
