...
**Bluetooth coexistence:** Meshtastic, MeshCore, and Reticulum may each use Bluetooth on **different devices** at once on all platforms. Same MAC is rejected. Only **active scans** are serialized; connected GATT links are not torn down for another protocol’s scan. LoRa GATT and Reticulum RNode/Peer share the sidecar `btleplug` central (concurrent sessions; no Noble / Web Bluetooth LoRa path).
