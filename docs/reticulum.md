...
5. **Chat** → LXMF direct messages. **Games** → Tic-Tac-Toe / Chess / Four in a Row over LRGP (or Challenge from Peers / Chat DM). **Remote** → rnsh shell + rncp file send/receive/fetch (high-speed paths). **RRC** → multi-hub relay chat. **Peers** and **Topology** for path-table visibility. **Nomad Network** → browse announced nodes or open **My Pages** to host a static Nomad site.
...
| Games           | LRGP Tic-Tac-Toe + Chess + Four in a Row via sibling [lrgp-rs](https://github.com/ratspeak/lrgp-rs); Games tab + Challenge from Peers/Chat; opponent labels via `resolveReticulumRemoteHashLabel`; deep-link `lrgp:<session>` / `lxm://game/<id>`; delivery chips + resend-after-restart (`games_outbound.db`); Chess promotion picker + threefold/50-move claims; wire-compatible with Ratspeak ([parity checklist](reticulum-games-parity.md))                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| Nomad Network   | Favourites / announces list (collapsible sidebar, default Favourites sub-tab) plus **My Pages** watched-folder hosting; **lazy-mount after first visit**; Micron (.mu) browser in a **dual-axis scroll shell**; **fit-width wrap default** with open-width toggle for ASCII pages; in-page navigation, back/forward, **in-memory** page + `/media` image LRUs (see [Nomad browser caches](#nomad-browser-caches)), `/file/` downloads, source toggle, and lxmf:// DM links; page/file errors humanized via `nomadPageErrorHumanize.ts`. Local hosting uses sibling [rsNomad](https://github.com/Colorado-Mesh/rsNomad) (`nomad-core`) for static `/page` + `/file` serving and `nomadnetwork.node` announces (no CGI). Choose a site root (`pages/`) or pages directory; FS watcher reloads routes; `nomad_serving_enabled` auto-restores after stack start. My Pages also supports in-app `.mu` authoring (new / edit / delete) via the **Micron page editor** (source + live preview); no file uploads.                                                                                                                                                                                               |
...
| Coexistence     | BLE on a **different** MAC from Meshtastic/MeshCore; scan mutex; LoRa GATT and RNode/Peer share the sidecar `btleplug` central (concurrent sessions; no Noble yield)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |

**Not in Reticulum mode:** Meshtastic/MeshCore-style RF channel chat, MQTT broker card, Meshtastic/MeshCore LoRa node position map, Rooms BBS, Meshtastic PKI Security tab, Hop Goblins routing diagnostics. (RRC is hub room chat over Reticulum Links — not LoRa RF channels.)
...
| Games           | LRGP Tic-Tac-Toe + Chess + Four in a Row (`GamesPanel`, `Gamepad2` icon); challenge/accept/play; Challenge also on Peers / Chat DM                                                                                                                                                                  |
...
| Nomad Network   | Favourites, announces, **My Pages** (watched-folder static host via rsNomad; auto-restore when stack starts), Micron page browser (dual-axis scroll shell, fit-width default + open-width toggle, navigation, in-memory page/image caches, file downloads); lazy-mount keep-alive after first visit |
...
| TAK             | Local CoT server and remote TAK relay. Reticulum contributes RMAP-discovered interfaces with coordinates and our own position from app static GPS; peers carry no position                                                                                                                          |
...
Hidden tabs (Meshtastic/MeshCore only): Modules/Repeaters, Rooms, Telemetry, Security, RF, Graph.
...
- **BLE RNode / BLE Peer:** runs `GET /api/v1/ble/scan` with `mode=rnode` or `mode=peer` (8 s timeout); may return `scan_busy` while Meshtastic/MeshCore hold an active GATT scan
- Sidecar exposes `GET /api/v1/ble/availability` for permission / adapter state; LoRa GATT uses `/api/v1/gatt/*` (see [Sidecar IPC](reticulum-sidecar-ipc.md))
...
**Bluetooth coexistence:** LoRa GATT and Reticulum RNode/Peer run in the same sidecar process with separate BLE centrals. The app checks configured Reticulum addresses against pending/live LoRa connections and serializes app-requested scans and LoRa connection setup. Links to other devices stay connected during a scan. Reticulum's autonomous discovery/reconnect remains outside that lease; behavior with concurrent links still depends on the OS and adapter.
...
- The selected room suppresses unread badges only while the app window is visible and focused. New messages still count as unread when that window is hidden, minimized, or behind another app. Returning to the room clears its unread count once the message view catches up to the bottom.
...
## Nomad browser caches

When **browsing** remote Nomad nodes (not hosting), the Micron viewer keeps two **renderer in-memory** LRUs so revisiting a page or remounting the panel can skip sidecar `/page` and `/media` fetches:

| Cache                        | Module               | Capacity    | Per-entry cap                      | Cleared when                                                                      |
| ---------------------------- | -------------------- | ----------- | ---------------------------------- | --------------------------------------------------------------------------------- |
| Page (Micron source)         | `nomadPageCache.ts`  | 128 entries | **256 KiB** text                   | Viewer **Clear page and image caches**, **Close page viewer**, or process restart |
| Image (`/media` WebP base64) | `nomadImageCache.ts` | 128 entries | **2 MiB** base64 (~1.5 MiB binary) | Same as page cache                                                                |

**Not persisted** to disk or SQLite — quitting the app or a full renderer reload drops both. Oversized pages or images are still shown when fetched, but are **not** stored in the LRU (so the next visit pays the mesh cost again). `force_path_refresh` / announce-retry loads bypass the image LRU so a forced path refresh still hits the network. Host-side conversion cache in rsNomad (`NomadNodeConfig.media_cache`) is separate and only applies when **this** node is serving `/media`.

Toolbar **⌀** clears both LRUs without closing the viewer; **✕** closes the viewer and also clears them.
...
- **LRGP games** — integrated via sibling [lrgp-rs](https://github.com/ratspeak/lrgp-rs) (`LrgpRouter` + `LrgpStore` in the sidecar). Reticulum **Games** tab (`Gamepad2`) for Tic-Tac-Toe, Chess, and Four in a Row; Challenge from Peers / Chat DM. Dedicated IPC `reticulum:games*` (generic proxy rejects `/api/v1/games/*`). WS `games.update` / `games.action_result`. Session `delivery_state` from LXMF outbound status; last envelope in `games_outbound.db` for Resend after restart. Notification / deep-link routes `lrgp:<session_id>` and `lxm://game/<session_id>` open the Games tab (`openReticulumGameSession`). Wire-compatible with Ratspeak; see [reticulum-games-parity.md](reticulum-games-parity.md).
