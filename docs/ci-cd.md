...
- **`scripts/test-linux-appimage-launch.mjs`** — headless launch smoke that proves the packaged Linux AppImage actually **boots**, not just that its files are well-formed. Runs under `xvfb-run` in the `packaging-smoke` Linux leg (`build.yaml` / `release.yaml`). Extracts the AppImage, launches its `AppRun` with `MESH_CLIENT_DISABLE_GPU=1`, `--no-sandbox --disable-setuid-sandbox`, and a throwaway `--user-data-dir`, then asserts the process stays alive past a startup threshold with no crash exit and that `mesh-client.log` in the user-data dir contains main-process (`[main]`) startup lines. Only the **native-arch** AppImage is launched (a cross-arch runtime cannot execute on the runner; that image is still covered by the sidecar extract smoke above). Requires `xvfb` + Electron runtime libs (`libnss3`, `libgtk-3-0t64`, `libgbm1`, `libasound2t64`, …), installed by the Linux smoke step.
