...
1. **`schema-release-compare`** — first job; compares this SHA’s `CURRENT_SCHEMA_VERSION` to the last **published** GitHub Release (by highest semver among non-draft/non-prerelease rows; recovers `vX.Y.Z` from release **name** when `tag_name` is `untagged-*`), writes the Actions step summary, and uploads a schema readme artifact. Job outputs feed installer notices and the draft release body.
