...
| GET    | `/api/v1/lxmf/recent`          | `?since_ts=` (ms, optional, **exclusive** lower bound), `?limit=` (default 200, max 500) | `{ messages: [], ring_len }` — ring buffer of recent **inbound** LXMF payloads for WS lag/reconnect catch-up (not durable across sidecar restart; capped at 200); `since_ts` keeps rows with `timestamp > since_ts` so a watermark equal to the newest ingested ts does not re-return that boundary forever; `ring_len` is current buffer occupancy                            |
