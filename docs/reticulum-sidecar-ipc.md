...
| POST   | `/api/v1/voice/audio`          | `{ profile?, channels, samples_b64 }` | Push one PCM frame (LE f32 base64) for Opus TX. Renderer must use dedicated IPC `reticulum:voiceSendAudio` (own ~2000/min budget); generic `reticulum:proxyPost` rejects this path so realtime PCM does not starve the shared 300/min proxy ceiling.                 |
