...
- **`peers_updated`:** also emitted when the live path table **gains** new destination hashes (maintenance tick). Payload may include `{ added: string[], patches: PeerRow[], count }` (added/patches capped at 1024). Renderer applies `patches` incrementally; full peer dump is reserved for connect / manual Refresh / restart / safety poll. Hop/timestamp-only churn does not emit.
