...
- **Drain policy** ([`useChatOutbox.ts`](../../src/renderer/hooks/useChatOutbox.ts) + App-level [`useEmergencyOutboxDrain.ts`](../../src/renderer/hooks/useEmergencyOutboxDrain.ts)):
...
- **Send helper:** [`emergencySend.ts`](../../src/renderer/lib/emergencySend.ts) — Reticulum live sends wait for LXMF receipt (or remote PN `propagated`) before reporting `'sent'`; mid-cascade `stored_locally` is not counted early (may not survive sidecar restart). Timeout/failure queues. DM-only with no destination throws (does not silently succeed).
