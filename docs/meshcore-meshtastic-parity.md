...
| Emoji reactions / tapbacks      | `reactions.ts` decodes protobuf tapbacks (`emoji` flag + UTF-8 payload, legacy index 1–12); `ChatPanel` quick picker + `sendReaction`                                                                                                    | Outbound companion `@[Name] emoji` via [`formatMeshcoreWireTapbackPrefix`](../src/renderer/lib/meshcoreChannelText.ts); inbound Open `r:HASH:INDEX` parsed in [`meshcoreOpenReaction.ts`](../src/renderer/lib/meshcoreOpenReaction.ts); echo dedup in [`meshcoreStoreDedup.ts`](../src/renderer/lib/meshcoreStoreDedup.ts)                  | **App** (shared UI, protocol-specific wire)                        |
