...
| PKC remote admin                | `ConfigureNodeSelector`, [`meshtasticRemoteAdmin.ts`](../src/renderer/lib/meshtasticRemoteAdmin.ts), `hasRemoteAdmin`; requires connected local radio (firmware 2.5+)                                                                    | Not available                                                                                                                                                                                                                                                                                                                               | **App** (Meshtastic-only)                                      |
...
- **Channel PSKs**: AES-128 (16-byte) or AES-256 (32-byte) base64 keys, one per line; optional `ChannelName=base64` for MQTT-only channels. LongFast default is always tried. Keys from the Radio tab sync when the radio is connected.
