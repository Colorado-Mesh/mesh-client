...
| Emoji reactions / tapbacks      | `reactions.ts` decodes protobuf tapbacks (`emoji` flag + UTF-8 payload, legacy index 1–12); `ChatPanel` quick picker + `sendReaction`                                                                                                    | MeshCore Open wire `r:HASH:INDEX` when the glyph is in the shared index table ([`meshcoreOpenReaction.ts`](../src/renderer/lib/meshcoreOpenReaction.ts)); legacy fallback `@[Name] emoji` via [`meshcoreChannelText.ts`](../src/renderer/lib/meshcoreChannelText.ts); echo dedup in [`meshcoreStoreDedup.ts`](../src/renderer/lib/meshcoreStoreDedup.ts) | **App** (shared UI, protocol-specific wire)                        |
