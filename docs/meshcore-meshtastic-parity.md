...
| Emoji reactions / tapbacks      | `reactions.ts` decodes protobuf tapbacks (`emoji` flag + UTF-8 payload, legacy index 1–12); `ChatPanel` quick picker + `sendReaction`                                                                                                    | Outbound keyless `@[Name] emoji` via [`buildMeshcoreOutboundTapbackWire`](../src/renderer/lib/meshcoreChannelText.ts) (text replies stay keyed); inbound keyed/keyless + Open `r:HASH:INDEX`; emoji-only replies promoted via [`meshcorePromoteEmojiOnlyReplyToTapback`](../src/renderer/lib/meshcoreChannelText.ts); echo dedup in [`meshcoreStoreDedup.ts`](../src/renderer/lib/meshcoreStoreDedup.ts) | **App** (shared UI, protocol-specific wire)                        |
...
| Regional flood scope            | Meshtastic region via LoRa config                                                                                                                                                                                                        | Radio tab **flood scope** (`setFloodScope` / `clearFloodScope`, hashtag presets, `app_settings` reapply on connect)                                                                                                                                                                                                                                                                                      | **App** (MeshCore v8+ transport keys)                              |
...
**Wire (MeshCore companion):** Tapbacks are text lines with a keyless bracket prefix and emoji-only body (not the keyed prefix used for thread replies):

```text
@[Display Name] emoji
```

Text replies remain keyed: `@[Display Name#replyKey] message`.

Outbound tapbacks use [`formatMeshcoreWireTapbackPrefix`](../src/renderer/lib/meshcoreChannelText.ts) + emoji via [`buildMeshcoreOutboundTapbackWire`](../src/renderer/lib/meshcoreChannelText.ts) in [`useMeshcoreRuntime`](../src/renderer/runtime/useMeshcoreRuntime.ts) `sendReaction`. Display names are sanitized via [`sanitizeMeshcoreWireName`](../src/renderer/lib/meshcoreChannelText.ts).

**Local model:** Stored rows use `emoji` + `replyId` + glyph-only `payload` — never the bracket wire string. Inbound replies with a single-emoji body are classified as tapbacks via [`meshcorePromoteEmojiOnlyReplyToTapback`](../src/renderer/lib/meshcoreChannelText.ts) (live ingest, repair, and display hydration).
...
| **Text replies** | Same wire prefix: `@[Name#replyKey] body` via [`formatMeshcoreWireReplyPrefix`](../src/renderer/lib/meshcoreChannelText.ts).                                                                                   |
| **Meshtastic**   | Unaffected — protobuf tapbacks (`replyId` + emoji flag), not MeshCore text lines.                                                                                                                              |
| **Dedup**        | mesh-client merges tapback RF/MQTT echoes ([`meshcoreStoreDedup.ts`](../src/renderer/lib/meshcoreStoreDedup.ts), 60 s window). Outbound tapbacks use keyless `@[Name] emoji` to match official companion wire. |

Inbound: parse `r:HASH:INDEX` (MeshCore Open), keyed `@[Name#key] emoji`, and keyless `@[Name] emoji` (legacy / other clients). See [troubleshooting.md — MeshCore duplicate chat messages](troubleshooting.md#meshcore-duplicate-chat-messages).
