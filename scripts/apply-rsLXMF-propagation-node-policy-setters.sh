...
echo "propagation-node policy setters patch did not apply on current HEAD; checking out pinned ref ${RS_LXMF_REF:0:12}"
if [[ -n "$(git -C "${LXMF_DIR}" status --porcelain)" ]]; then
  ...
  echo "Stash/commit sibling changes, or ensure the policy setters are already present." >&2
  exit 1
fi
...
if overlay_already_present; then
  echo "propagation-node policy setters overlay already present on rsLXMF @ ${RS_LXMF_REF:0:12}"
  exit 0
fi
