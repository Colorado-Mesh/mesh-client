...
describe('ChatPanel MeshCore waiting-message status', () => {
  const meshcoreProps = {
    messages: [],
    channels: [{ index: 0, name: 'General' }],
    myNodeNum: 1,
  ...
  it('shows silent drain banner on serial while auto-drain is active', () => {
    render(
      <ToastProvider>
        <ChatPanel {...meshcoreProps} waitingMessagesSilentDrainActive />
      </ToastProvider>,
    );
    const status = screen.getByRole('status');
    expect(status).toHaveTextContent(/Fetching messages queued on the radio/i);
    expect(status).toHaveTextContent(/USB serial handles one command at a time/i);
  });

  it('hides silent drain banner when idle', () => {
    render(
      <ToastProvider>
        <ChatPanel {...meshcoreProps} />
      </ToastProvider>,
    );
