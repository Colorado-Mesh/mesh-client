...
describe('Reticulum queue header cluster', () => {
  ...
  it('shows Q badge without spinner when empty', () => {
    render(<ReticulumQueueHeaderCluster free={256} maxlen={256} interfaceName="RNode USB" />);
    expect(screen.getByText('Q: 0/256')).toBeInTheDocument();
    expect(screen.queryByRole('status')).not.toBeInTheDocument();
  });

  it('shows buffering spinner and Q badge when fill > 0', () => {
    render(<ReticulumQueueHeaderCluster free={192} maxlen={256} interfaceName="RNode 41F4" />);
