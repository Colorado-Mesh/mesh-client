...
class IpcNobleConnection {
  ...
  async connect(): Promise<void> {
    const runConnect = async () => {
      ...
      const instance = new NobleOverIpc(
        sessionId,
      ) as unknown as NobleIpcMeshcoreConnectionInstance;
      this.inner = instance;
      ...
      try {
        await withTimeout(
          window.electronAPI
            .connectNobleBle(sessionId, this.peripheralId)
            .then((result) => {
...
async function connectBleWebBluetooth(): Promise<Connection> {
  ...
  for (let attempt = 1; attempt <= WEB_BLUETOOTH_CONNECT_MAX_ATTEMPTS; attempt++) {
    ...
    try {
      ...
      reuseDeviceId =
        deviceInfo?.deviceId ?? transport.getLastGrantedDeviceId() ?? reuseDeviceId;
      try {
