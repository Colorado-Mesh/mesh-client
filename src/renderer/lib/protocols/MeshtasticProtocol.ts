...
export class MeshtasticProtocol implements Protocol {
  ...
  subscribe(handle: unknown, emit: (event: DomainEvent) => void): () => void {
    ...
    push(device.events.onDeviceStatus.subscribe((status) => fire(this.decodeDeviceStatus(status))));
    push(
    ...
    push(device.events.onNodeInfoPacket.subscribe((p) => fire(this.decodeNodeInfo(p))));
    push(device.events.onPositionPacket.subscribe((p) => fire(this.decodePosition(p))));
    push(device.events.onTelemetryPacket.subscribe((p) => fire(this.decodeTelemetry(p))));
    push(device.events.onWaypointPacket.subscribe((p) => fire(this.decodeWaypoint(p))));
    push(device.events.onTraceRoutePacket.subscribe((p) => fire(this.decodeTraceRoute(p))));
    push(device.events.onChannelPacket.subscribe((p) => fire(this.decodeChannel(p))));
    push(device.events.onConfigPacket.subscribe((p) => fire(this.decodeConfig(p))));
    push(device.events.onModuleConfigPacket.subscribe((p) => fire(this.decodeModuleConfig(p))));
    push(device.events.onQueueStatus.subscribe((p) => fire(this.decodeQueueStatus(p))));
    push(device.events.onLogRecord.subscribe((p) => fire(this.decodeDeviceLog(p))));
    push(device.events.onDeviceMetadataPacket.subscribe((p) => fire(this.decodeDeviceMetadata(p))));
    push(device.events.onNeighborInfoPacket.subscribe((p) => fire(this.decodeNeighborInfo(p))));
    push(
  ...
  async setModuleConfig(handle: unknown, config: unknown): Promise<void> {
    await (
      handle as unknown as { setModuleConfig: (c: unknown) => Promise<void> }
    ).setModuleConfig(config);
  }

  async setCannedMessages(handle: unknown, messages: string[]): Promise<void> {
    await (
      handle as unknown as { setCannedMessages: (m: { messages: string }) => Promise<void> }
  ...
  async deleteNode(_handle: unknown, _nodeId: number): Promise<void> {
    throw new UnsupportedOperation('meshtastic deleteNode');
  }
