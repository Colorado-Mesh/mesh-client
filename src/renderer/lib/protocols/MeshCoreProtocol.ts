...
export class MeshCoreProtocol implements Protocol {
  ...
  subscribe(handle: unknown, emit: (event: DomainEvent) => void): () => void {
    ...
    const onDm = (data: unknown) => this.decodeDirectMessage(data, nodeIdByPrefix).forEach(emit);
    const onChannel = (data: unknown) => this.decodeChannelMessage(data).forEach(emit);
  ...
  identitySignature(params: TransportParams, info?: DiscoveryInfo): string {
    if (info?.publicKey && info.publicKey.length === 32) {
  ...
  async discoverSelf(handle: unknown, timeoutMs = 5000): Promise<DiscoveryInfo> {
    const bus = handle as unknown as MeshCoreEventBus;
  ...
  async sendTraceRoute(_handle: unknown, _nodeId: number): Promise<void> {
    throw new UnsupportedOperation('meshcore sendTraceRoute (use Connection.requestPath in step 2d)');
  }
  ...
  async deleteNode(_handle: unknown, _nodeId: number): Promise<void> {
    throw new UnsupportedOperation('meshcore deleteNode (step 2d)');
  }
  ...
  async syncClock(handle: unknown): Promise<void> {
    ...
    await (handle as unknown as { syncDeviceTime: () => Promise<void> }).syncDeviceTime();
  }
  ...
  async signData(handle: unknown, data: Uint8Array): Promise<Uint8Array> {
    const conn = handle as Connection;
    const result = (await conn.sign(data)) as Uint8Array;
    return result;
  }
  ...
  private decodeAdvert(
    ...
    const events: DomainEvent[] = [
      { type: 'node_info', payload: { nodeId, longName: d.advName, lastHeardAt: d.lastAdvert, publicKey: d.publicKey } },
    ];
