...
function handleTextMessage(
  ...
  const displayName = record.senderName?.trim() || 'Unknown';
  const senderId =
  ...
  const nextRecord = chatMessageToMessageRecord(merged);
  nextRecord.id = event.payload.id;
  upsertMessage(identityId, nextRecord);

  const isEcho = myNodeNum > 0 && senderId === myNodeNum;
  if (!isEcho) {
    void window.electronAPI.db.saveMeshcoreMessage(messageToDbRow(merged)).catch((e: unknown) => {
