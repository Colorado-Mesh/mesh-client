...
describe('meshcoreRfPath parity with main decoder fixtures', () => {
  it('decodes flood PATH packets', () => {
    ...
    expect(isMeshCorePathPacketByte0(buffer[0])).toBe(true);
    expect(isMeshCoreTracePacketByte0(buffer[0])).toBe(false);
  });
  ...
  it('decodes TRACE header labels', () => {
    ...
    expect(meshCorePayloadTypeStringFromByte0(buffer[0])).toBe('TRACE');
    expect(meshCoreRouteTypeStringFromByte0(buffer[0])).toBe('FLOOD');
    expect(isMeshCoreTracePacketByte0(buffer[0])).toBe(true);
    const result = decodeMeshCorePathPrefix(buffer);
    expect(result.hops).toBe(3);
    expect(result.path).toEqual([0xaa, 0xbb, 0xcc]);
  });
