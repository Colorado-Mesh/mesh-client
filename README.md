...
- **Waiting-message drain**: header status indicator (all protocol tabs when MeshCore has queued messages) during serial companion backlog drain; **Sync now** for manual catch-up
...
- **MeshCore - Trace Route / Ping trace**: Remote nodes typically respond only if they have **your** node as a contact. One-way or foreign heard nodes may lead to a client-side timeout; multi-hop paths without synced outPath bytes trigger up to **two** flood-advert priming rounds (**15s + 5s × hops** wait per round, capped at **45s**) before `SendTracePath`, or a fast **No route from radio yet** error when priming cannot build a hash-segment path. See [Troubleshooting; MeshCore: Trace Route or Ping trace times out](docs/troubleshooting.md#meshcore-trace-route-or-ping-trace-times-out).
