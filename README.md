...
- **Remote TAK server relay**: stream the same positions to an OpenTAKServer, FreeTAKServer, or TAK Server over mutual TLS. Import the server truststore and your client certificate as `.p12` or PEM files; the private key stays in the main process. The server certificate must chain to the imported CA and name the server address (an opt-out accepts a certificate issued for another name, as ATAK does); the relay reconnects on its own and can connect at launch
