...
| Boundary     | Path                | Role                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------ | ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Main         | `src/main/`         | SQLite (`database.ts`, `db-compat.ts`), BLE (`noble-ble-manager.ts`), MQTT (`mqtt-manager.ts`, `meshcore-mqtt-adapter.ts`), logging (`log-service.ts`, `sanitize-log-message.ts`), IPC handlers (`index.ts` plus namespaced modules in `src/main/ipc/` — Reticulum, Reticulum DB, Reticulum identity, RRC DB, TAK, GPS), window, GPS, updater, headless remote HTTP/WS (`headless/`) |
...
| Shared       | `src/shared/`       | IPC contracts (`electron-api.types.ts`), protocol-neutral helpers (`headless.ts`, `remoteProtocol.ts`, …)                                                                                                                                                                                                                                                                            |
...
| Headless server mode, Docker image, HTTP/WS remote control                                    | [`docs/agents/headless.md`](docs/agents/headless.md)                     |
...
- Headless/server mode (`MESH_CLIENT_HEADLESS` or container detect) requires `MESH_CLIENT_REMOTE_TOKEN` for non-loopback binds; remote-control HTML is English-only (not i18n-gated) — see [`docs/agents/headless.md`](docs/agents/headless.md) and [`docs/headless-server.md`](docs/headless-server.md).
