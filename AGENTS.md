...
**App wiring:** Prefer `useProtocolFacade(protocol)` for connection state, panel actions, nodes, and messages. Use per-protocol `useProtocolConnectionActions('meshtastic' | 'meshcore' | 'reticulum')` when ConnectionPanel props differ per tab. **`usePowerRecovery`** mounts once from `App.tsx` — coordinates sleep/wake IPC, MQTT `powerSuspend`/`powerResume`, runtime `onPowerResume` (Meshtastic ~4s after wake, MeshCore ~8s stagger after Meshtastic, Reticulum sidecar resume, plus up to 30s dual-Noble BLE settle when Meshtastic and MeshCore both use BLE), and **`useRendererHeartbeat`** (renderer pings main every 30s; main warns if no heartbeat within 30s after `powerMonitor` resume).
...
- **Support bundle IPC:** `support:exportBundle` — `exportSupportBundle.ts` → `window.electronAPI.support.exportBundle(mode, json)`; main `support-bundle.ts` writes zip (`github` = logs + debug snapshot; `developer` = includes SQLite). Modes in `support-bundle.types.ts`.
...
- **Per-repeater passwords:** `meshcoreRepeaterCredentialStorage.ts` (`meshcoreRepeaterCredential:<nodeId>` in `app_settings` via IPC), `useMeshcoreRepeaterRemoteAuth.tsx`, `MeshcoreRepeaterPasswordControls.tsx`; Repeaters panel saved-password list + Forget (parallel to Rooms).
- **Waiting-message drain:** event 131 → `meshcoreWaitingMessagesDrain.ts` / `meshcoreProcessWaitingMessageItem.ts`; silent auto-drain vs manual **Sync now** (`MeshcoreWaitingMessagesBanner.tsx` on Chat/Rooms); defers during TraceData/admin RPC.
