...
- **Reactions / tapbacks:** `reactions.ts` (Meshtastic protobuf) + `meshcoreOpenReaction.ts` (MeshCore Open `r:HASH:INDEX`; interop-only picker in `MeshcoreReactionPicker.tsx`); inbound legacy `@[Name] emoji` still parsed in `meshcoreChannelText.ts`; `ChatPanel.tsx` attaches tapbacks via `replyId` + runtime/panel `sendReaction`.
